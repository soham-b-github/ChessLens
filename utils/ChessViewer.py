...
''' Modified code w/ single FEN and no delay (from ChatGPT ofc) '''        
        
        
        
class ChessViewer(ChessBoard, ChessPlay):
    def __init__(self, fen, white_player, black_player):
        ChessBoard.__init__(self)
        ChessPlay.__init__(self)
        ...
        self.black_clock_frame = tk.Frame(self.main_frame)
        self.black_clock_frame.grid(row=0, column=2, padx=20, sticky='n')
        self.white_clock_frame = tk.Frame(self.main_frame)
        self.white_clock_frame.grid(row=1, column=2, padx=20, sticky='s')
        ...
        self.play_button = tk.Button(self.button_frame, text="Play", state="disabled")
        self.play_button.grid(row=0, column=2, padx=5)
        
        self.stop_button = tk.Button(self.button_frame, text="Stop", state="disabled")
        self.stop_button.grid(row=0, column=3, padx=5)
    ...
    def display_position(self):
        """Display the provided single FEN position."""
        is_valid, result = self.validate_fen(self.fen)
        ...
        active_player = self.get_active_player(self.fen)
        if active_player == 'w' and self.white_time > 0:
            self.white_time -= 1
            if self.use_increment:
                self.white_time += self.increment
